...
//         ref: item.txRef,
//         transactionId: item.transactionId,
//         date: item.createdAt || item.timestamp || item.unlockedAt, 
//         status: "success",
//       }));
...
allTx.sort((a, b) => {
  ...
  return getTime(b) - getTime(a);
    ...
    console.log("📝 Paginated sample:");
    paginatedTx.forEach((tx, i) => {
      console.log(`   Pg[${i}]`, tx.type, tx.date);
    });
...
router.delete('/delete-account/:userId', async (req, res) => {
  ...
  try {
    ...
    // 3️⃣ Delete all device records containing this user
    await Device.deleteMany({ "users._id": userId });

    res.status(200).json({ message: 'Account & device deleted successfully' });
  } catch (error) {
